...
document.addEventListener('DOMContentLoaded', () => { // Add an event listener to the DOMContentLoaded event to start the quiz
    ...
    function showQuestion(question) {
        ...
        shuffleArray(question.answers).forEach(answer => { // Shuffle the answers
    ...
    function selectAnswer(e) {
        ...
        setTimeout(() => { // Set a timeout of 1 second
            currentQuestionIndex++ // Increment the index of the current question
            if (currentQuestionIndex < shuffledQuestions.length) { // If the index of the current question is less than the length of the shuffled questions array
                setNextQuestion() // Set the next question
            } else { // Otherwise
                endQuiz() // End the quiz
            }
    ...
    function disableButtons() {
        Array.from(elements.answerButtons.children).forEach(button => { // Loop through the children of the answer buttons
    ...
    function saveHighScore() {
        ...
        elements.form.onsubmit = e => { // Add an event listener to the form
            e.preventDefault() // Prevent the default form submission behavior
            const initials = elements.input.value // Get the value of the input element
            if (initials.trim() !== '') { // If the input value is not empty
    ...
    function toggleHighScores() {
        ...
        if (isHidden) { // If the high scores container is hidden 
            showHighScores() // Show the high scores
        } else {
